// src/ts/components/home/PopularSection.ts
...
class PopularSection extends HTMLElement {
  ...
  constructor() {
    super();
    console.log("PopularSection: constructor called");
  }

  async connectedCallback(): Promise<void> {
    console.log("PopularSection: connectedCallback called");
    this.render();
    ...
    this.innerHTML = `
      <section class="mt-[32px] ps-[104px] relative">
        <div class="flex gap-[16px] items-center px-[20px]">
          <h2 class="text-[24px] font-bold text-black">Popular</h2>
  ...
  async fetchAndRenderContents(): Promise<void> {
    ...
    try {
      ...
      let contents: Movie[] | Serie[] = [];

      if (this.currentCategory === 'movie') {
        contents = data.results as Movie[];
      } else {
        contents = data.results as Serie[];
      }

      if (contents && contents.length > 0) {
        contents.sort((a: any, b: any) => b.vote_average - a.vote_average);

        const contentsHTML = contents
          .map((content: Movie | Serie) => {
        ...
        const container = this.querySelector('#contents-container');
        if (container) {
          container.innerHTML = `<p class="text-center text-gray-500">No popular ${this.currentCategory === 'movie' ? 'movies' : 'series'} available.</p>`;
        }
      }
    } catch (error) {
      console.error(`Erreur lors de la récupération des contenus populaires (${this.currentCategory}):`, error);
      const container = this.querySelector('#contents-container');
      if (container) {
        container.innerHTML = `<p class="text-center text-red-500">Failed to load popular ${this.currentCategory === 'movie' ? 'movies' : 'series'}. Please try again later.</p>`;
      }
    }
  }
}

if (!customElements.get("popular-section")) {
